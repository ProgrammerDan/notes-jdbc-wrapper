 ...
 */
public class LNViewListResultSet implements ResultSet {
    ...
    protected static String toString(ViewColumn col){
    	String s="";
    	try {
			s+=col.getTitle();
			s+=":";
			if(col.isSorted())s+="S";
			if(col.isCategory())s+="C";
    	}catch(Exception e){
    		s=""+e;
    	}
		return s;
    ...
    protected static String toString(Vector<ViewColumn> cols){
    	String s="[";
		for(int i=0;i<cols.size();i++){
			ViewColumn col=(ViewColumn)cols.get(i);
			if(i>0)s+=", ";
			s+=toString(col);
		}
		s+="]";
    ...
    protected LNViewListResultSet(Statement st, Vector<View> views) throws Exception {
        this.statement = st;
        ...
        header.add("columns");
 		...
 		md = new LNResultSetMetaData(new Vector(header));

		for(int i=0;i<views.size();i++) {
			View view=views.get(i);
			List row = new ArrayList();
			row.add(view.getName());
			row.add(toString(view.getColumns()));
			data.add(row);
		}
    }
    ...
    public java.sql.Date getDate(int columnIndex) throws SQLException {
        java.util.Date value = (java.util.Date)getObject(columnIndex);
		if (value == null){
			return null;
		}else{
