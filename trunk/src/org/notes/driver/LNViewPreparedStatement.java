 ...
 */
 ...
 /* implements the following sql: SELECT VIEW {view-name} {?param-name-1} {?param-name-2} ... */
 /* we need param name to check if we can filter by this item (must be sorted column in the view) */
 
public class LNViewPreparedStatement extends LNStatement implements PreparedStatement {
	
	private Map param = new HashMap();
	private List<String> viewParam=new ArrayList<String>();
	private String viewName=null;
	private String sql=null;
	
	protected LNViewPreparedStatement(LNConnection con,String sql)throws SQLException{
		super(con);
	    prepare(sql);
	}
	
	protected void prepare(String sql)throws SQLException{
		if(sql==null)throw new LNException("Can't prepare sql: "+sql);
		
		if(sql.equals(this.sql)){
			//this query already prepared
			return;
		}
		this.sql=sql;
		
		//let's prepare a new sql...
		viewParam.clear();
		viewName = null;
        
		if (sql.matches("\\s*SELECT\\s+VIEW\\s+.*")) {
			viewName = sql.replaceAll("\\s*SELECT\\s+VIEW\\s+(.*)", "$1").trim();
		}else{
			throw new LNException("Can't prepare Notes View Request: wrong query " + sql);
		}
		
		if(viewName==null)throw new LNException("Can't prepare Notes View Request: view name absent: " + sql);
	}
		...
		try {	
			...
			if( param.size()>0 ) {
				...
				for(int i=0;i<cols.size();i++){
					...
					if(col.isSorted()){
						icol++;
						if(param.containsKey(icol)){
							Object value=param.get(icol);
							if(value==null)value="";
							filter.add(value);
						}else break;
					}
			...
			throw new LNException("Notes Execute error",e);
		}
	}
	 ...
	 * this method is called on a closed <code>PreparedStatement</code>
	 * @exception LNFeatureNotSupportedException if <code>sqlType</code> is
     ...
     * marker in the SQL statement; if a database access error occurs or 
     * this method is called on a closed <code>PreparedStatement</code>
     * @exception LNFeatureNotSupportedException if the JDBC driver does not support
     * this method
     * @deprecated
     */
    ...
    * @exception SQLException if parameterIndex does not correspond to a parameter
     * marker in the SQL statement; if a database access error occurs or 
    * this method is called on a closed <code>PreparedStatement</code>
    * @exception LNFeatureNotSupportedException if <code>targetSqlType</code> is
    * a <code>ARRAY</code>, <code>BLOB</code>, <code>CLOB</code>, 
    * <code>DATALINK</code>, <code>JAVA_OBJECT</code>, <code>NCHAR</code>, 
    * <code>NCLOB</code>, <code>NVARCHAR</code>, <code>LONGNVARCHAR</code>,
    *  <code>REF</code>, <code>ROWID</code>, <code>SQLXML</code>
    ...
    */
     ...
     * marker in the SQL statement; if a database access error occurs or 
     * this method is called on a closed <code>PreparedStatement</code>
     * @throws LNFeatureNotSupportedException  if the JDBC driver does not support this method
     * @since 1.2
     */
